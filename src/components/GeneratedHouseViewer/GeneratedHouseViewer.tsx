...
interface Props {
  houseId: string;
  /** Optional cache-buster (e.g. the `ts` returned by /api/generate). */
  bust?: string | number;
}

export function GeneratedHouseViewer({ houseId, bust }: Props) {
  const suffix = bust !== undefined ? `?ts=${bust}` : '';
  const url = `/baked/${houseId as HouseId}-generated-latest.glb${suffix}`;
