...
export function ObliqueCesium() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Cesium 1.140's typings make several scene props optional even though
      // they exist at runtime; cast `scene` to a loose record so we can hide
      // sky/sun/moon/fog without piling on `if (x)` guards.
      const scene = viewer.scene as unknown as {
        ...
        fog: { enabled: boolean };
        globe: { show: boolean };
        backgroundColor: import('cesium').Color;
      };
