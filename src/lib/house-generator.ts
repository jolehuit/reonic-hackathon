...
export async function generateHouse(input: GenerateInput): Promise<GenerateResult> {
  ...
  const W = REQ_W * SCALE;
  const H = REQ_H * SCALE - ATTRIBUTION_PX;
  const cropped = await sharp(Buffer.from(await topDownRes.arrayBuffer()))
    .extract({ left: 0, top: 0, width: W, height: H })
    .png()
    .toBuffer();

  // ── 3D oblique view via the /api/aerial?tilted=1 route ──
  // That route launches headless Chromium against /oblique (Cesium + Google
  // Photorealistic 3D Tiles) and screenshots it. Reusing the existing route
  // keeps the screenshot logic in one place.
  let tiltedBuf: Buffer | null = null;
  if (input.tilted) {
    tiltedBuf = await fetchObliqueScreenshot(input.lat, input.lng, zoom, input.origin);
  }
  ...
  if (tiltedBuf) {
    ...
    // Overlay a red disc at image centre so the AI has a consistent "pin"
    // reference (downstream prompts depend on it).
...
// Screenshots /oblique via the existing /api/aerial?tilted=1 route. That route
// does Playwright + Cesium + Google Photorealistic 3D Tiles in one place; we
// piggyback on it so the screenshot logic isn't duplicated here.
async function fetchObliqueScreenshot(
  lat: number,
  lng: number,
  zoom: number,
  origin?: string,
  ...
  const url = `${origin}/api/aerial?lat=${lat}&lng=${lng}&zoom=${zoom}&tilted=1`;
  const res = await fetch(url);
  if (!res.ok) return null;
  return Buffer.from(await res.arrayBuffer());
}

// Composites a small solid red disc with a white outline at the image centre.
// Mirrors the Static Maps `markers=color:red|size:tiny` pin so the Gemini
// prompts see a consistent visual anchor between top-down and oblique views.
async function overlayCenterPin(buf: Buffer, w: number, h: number): Promise<Buffer> {
  ...
  const svg = Buffer.from(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}">` +
      ...
      `<circle cx="${cx}" cy="${cy}" r="${r}" fill="#ff1f1f"/>` +
      `</svg>`,
  );
  return await sharp(buf).composite([{ input: svg, top: 0, left: 0 }]).png().toBuffer();
}
